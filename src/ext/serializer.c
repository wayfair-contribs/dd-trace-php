...
void _serialize_exception(zval *el, zval *meta, ddtrace_span_t *span) {
    ...
    add_assoc_stringl(meta, "error.name", class_name, class_name_len, dup);
    _add_assoc_zval(meta, "error.msg", msg);
    _add_assoc_zval(meta, "error.stack", stack);
}

void _serialize_meta(zval *el, ddtrace_span_t *span) {
    zval *meta = _read_span_property(span->span_data, "meta", sizeof("meta") - 1);
...
#define ADD_ELEMENT_IF_PROP_TYPE(name, type)                                       \
    do {                                                                           \
        zval *prop = _read_span_property(span->span_data, name, sizeof(name) - 1); \
