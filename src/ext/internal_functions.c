...
ZEND_NAMED_FUNCTION(ddtrace_hander_curl_exec) {
    ...
    ddtrace_span_ids_t *root_span_id = ddtrace_root_span_id(TSRMLS_C);
    ddtrace_span_ids_t *active_span_id = ddtrace_active_span_id(TSRMLS_C);
    // No trace to propagate
    if (!root_span_id || !active_span_id) {
        orig_handler_curl_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU);
        return;
    }
    ...
    */
    ...
    snprintf(header_trace_id, sizeof(header_trace_id), "x-datadog-trace-id: %" PRIu64, root_span_id->id);
